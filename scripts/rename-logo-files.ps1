# PowerShell script to rename logo files from token symbols to contract addresses

# Token mapping
$tokenMap = @{
    "pht" = "0x885c99a787BE6b41cbf964174C771A9f7ec48e04"
    "wkc" = "0x6Ec90334d89dBdc89E08A133271be3d104128Edb"
    "war" = "0x57bfe2af99aeb7a3de3bc0c42c22353742bfd20d"
    "dtg" = "0xb1957BDbA889686EbdE631DF970ecE6A7571A1B6"
    "yukan" = "0xd086B849a71867731D74D6bB5Df4f640de900171"
    "btcdragon" = "0x1ee8a2f28586e542af677eb15fd00430f98d8fd8"
    "ocicat" = "0xE53D384Cf33294C1882227ae4f90D64cF2a5dB70"
    "nene" = "0x551877C1A3378c3A4b697bE7f5f7111E88Ab4Af3"
    "twc" = "0xDA1060158F7D593667cCE0a15DB346BB3FfB3596"
    "tkc" = "0x06Dc293c250e2fB2416A4276d291803fc74fb9B5"
    "durt" = "0x48a510A3394C2A07506d10910EBEFf3E25b7a3f1"
    "twd" = "0xf00cD9366A13e725AB6764EE6FC8Bd21dA22786e"
    "gtan" = "0xbD7909318b9Ca4ff140B840F69bB310a785d1095"
    "zedek" = "0xCbEaaD74dcB3a4227D0E6e67302402E06c119271"
    "bengcat" = "0xD000815DB567372C3C3d7070bEF9fB7a9532F9e8"
    "bcat" = "0x47a9B109Cfb8f89D16e8B34036150eE112572435"
    "nct" = "0x9F1f27179fB25F11e1F8113Be830cfF5926C4605"
    "kitsune" = "0xb6623B503d269f415B9B5c60CDDa3Aa4fE34Fd22"
    "crystalstones" = "0xe252FCb1Aa2E0876E9B5f3eD1e15B9b4d11A0b00"
    "bft" = "0x4b87F578d6FaBf381f43bd2197fBB2A877da6ef8"
    "cross" = "0x72928a49c4E88F382b0b6fF3E561F56Dd75485F9"
    "thc" = "0x56083560594E314b5cDd1680eC6a493bb851BBd8"
    "bbft" = "0xfB69e2d3d673A8DB9Fa74ffc036A8Cf641255769"
    "bob" = "0x51363f073b1e4920fda7aa9e9d84ba97ede1560e"
    "surv" = "0xAfF713b62e642b25898e24d5Be6561f863582144"
    "tut" = "0xCAAE2A2F939F51d97CdFa9A86e79e3F085b799f3"
    "puffcat" = "0x14a8d0AC8Fc456899F2DD33C3f4E32403A78126c"
    "crepe" = "0xeb2B7d5691878627eff20492cA7c9a71228d931D"
    "popielno" = "0xdc3d92dd5a468edb7a7772452700cc93bb1826ad"
    "spray" = "0x6C0D4adAc8fb85CC336C669C08b44f2e1d492575"
    "mbc" = "0x170f044f9c7a41ff83caccad6ccca1b941d75af7"
    "mars" = "0x6844b2e9afb002d188a072a3ef0fbb068650f214"
    "sdc" = "0x8cDC41236C567511f84C12Da10805cF50Dcdc27b"
    "kind" = "0x41f52A42091A6B2146561bF05b722Ad1d0e46f8b"
    "shibc" = "0x456B1049bA12f906326891486B2BA93e46Ae0369"
    "pcat" = "0xFeD56F9Cd29F44e7C61c396DAc95cb3ed33d3546"
    "persiancat" = "0x71fd83d49fAaD4612E9d35876A75a97a5aDd4Bc2"
    "egw" = "0x2056d14A4116A7165cfeb7D79dB760a06b57DBCa"
    "1000pdf" = "0xCa7930478492CDe4Be997FA898Cd1a6AfB8F41A1"
    "aidove" = "0xe9E3CDB871D315fEE80aF4c9FcD4886782694856"
    "hmt" = "0x360f2cf415d9be6e82a7252681ac116fb63d2fa2"
    "rbcat" = "0x14A2db256Ef18c4f7165d5E48f65a528b4155100"
    "bbcat" = "0x32Eb603F30ba75052f608CFcbAC45e39B5eF9beC"
    "cct" = "0x8489c022a10a8d2a65eb5aF2b0E4aE0191e7916D"
    "talent" = "0x38Aec84f305564cB2625430A294382Cf33e3c317"
    "peso" = "0xC54CA14328d5b61E4BDc8A4d4b08b6B8D06BC372"
}

$logoDir = "public\images\bsc\token-logos"

Write-Host "🔄 Starting logo file renaming process..." -ForegroundColor Yellow
Write-Host ""

if (-not (Test-Path $logoDir)) {
    Write-Host "❌ Logo directory not found: $logoDir" -ForegroundColor Red
    exit 1
}

$files = Get-ChildItem -Path $logoDir -File
$renamedCount = 0
$skippedCount = 0
$errorCount = 0

foreach ($file in $files) {
    # Skip non-image files and files that are already contract addresses
    if ($file.Name.StartsWith("0x") -or $file.Name.Contains(".docx") -or $file.Name -eq "Untitled-1.png") {
        Write-Host "⏭️  Skipping: $($file.Name) (already processed or non-token file)" -ForegroundColor Gray
        $skippedCount++
        continue
    }

    # Extract symbol from filename (remove extension)
    $symbol = [System.IO.Path]::GetFileNameWithoutExtension($file.Name).ToLower()
    
    # Find corresponding contract address
    $contractAddress = $tokenMap[$symbol]
    
    if (-not $contractAddress) {
        Write-Host "⚠️  No contract address found for symbol: $symbol" -ForegroundColor Yellow
        $skippedCount++
        continue
    }

    # Create new filename with contract address
    $newFilename = "$($contractAddress.ToLower())$($file.Extension)"
    $newPath = Join-Path $logoDir $newFilename

    try {
        # Check if target file already exists
        if (Test-Path $newPath) {
            Write-Host "⚠️  Target file already exists: $newFilename" -ForegroundColor Yellow
            $skippedCount++
            continue
        }

        # Rename the file
        Rename-Item -Path $file.FullName -NewName $newFilename
        Write-Host "✅ Renamed: $($file.Name) → $newFilename" -ForegroundColor Green
        $renamedCount++
    }
    catch {
        Write-Host "❌ Error renaming $($file.Name): $($_.Exception.Message)" -ForegroundColor Red
        $errorCount++
    }
}

Write-Host ""
Write-Host "📊 Renaming Summary:" -ForegroundColor Cyan
Write-Host "✅ Successfully renamed: $renamedCount files" -ForegroundColor Green
Write-Host "⏭️  Skipped: $skippedCount files" -ForegroundColor Yellow
Write-Host "❌ Errors: $errorCount files" -ForegroundColor Red
Write-Host ""
Write-Host "🎉 Logo file renaming process completed!" -ForegroundColor Green
