...
export async function GET(request: Request) {
  ...
  try {
    ...
    // Normalize response to match frontend expectations and filter for BSC pages
    const normalized = {
      ...
      pages: Array.isArray(data?.pages)
        ? data.pages
            .filter((p: any) => {
              const page = p?.page ?? p?.value ?? "";
              return page.includes('/bsc/');
            })
            .map((p: any) => ({
              page: (p?.page ?? p?.value ?? "").replace('/bsc/', ''),
    ...
    // Import TOKEN_MAP from ca route to get total tokens
    const TOKEN_MAP: Record<string, { address: string }> = {
    ...
    return corsResponse({
      ...
      totalTokens: Object.keys(TOKEN_MAP).length,
      // tokensList: Object.keys(TOKEN_MAP)
    }, 200);
  } catch (error) {
